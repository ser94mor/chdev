...
static ssize_t         chdev_read_common(struct chdev_dev *, char __user *, size_t);
static ssize_t         chdev_write_common(struct chdev_dev *, const char __user *, size_t);
 ...
 */
static ssize_t chdev_read_common(struct chdev_dev *dev, char __user *buf, size_t count) {
 ...
 */
static ssize_t chdev_write_common(struct chdev_dev *dev, const char __user *buf, size_t count) {
    ...
    if (chdev->inv) {
        ...
        if (count + sizeof(short) <= FREE_SPACE_DOWNSIDE) {
            ...
            if (sizeof(short) >= FREE_SPACE_DOWNSIDE) {
                /* write item length */
                memcpy(chdev->end, (const char *)&item_len, FREE_SPACE_DOWNSIDE);
                memcpy(chdev->buf, (const char *)&item_len + FREE_SPACE_DOWNSIDE, sizeof(short) - FREE_SPACE_DOWNSIDE);
                
                /* copy item from user */ 
                if (copy_from_user(chdev->buf + sizeof(short) - FREE_SPACE_DOWNSIDE, buf, count)) {
                    return -EFAULT;
                }
                ...
                /* copy item from user */ 
                if (copy_from_user(chdev->end + sizeof(short), buf, FREE_SPACE_DOWNSIDE - sizeof(short))) {
                    return -EFAULT;
                }
                if (copy_from_user(chdev->buf, buf + FREE_SPACE_DOWNSIDE - sizeof(short), count - FREE_SPACE_DOWNSIDE + sizeof(short))) {
                    return -EFAULT;
                }
